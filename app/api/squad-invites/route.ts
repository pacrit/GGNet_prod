...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isNaN(inviteIdNum) || inviteIdNum <= 0) {
    ...
    if (action === 'accept') {
      ...
      await sql`
        ...
        VALUES (${squadId}::integer, ${userId}::integer, 'member'::text)
      `;

      // Criar notificação para quem convidou
      const acceptNotificationType = 'squad_invite_accepted';
      const acceptNotificationMessage = `Seu convite para o squad "${invite[0].squad_name}" foi aceito!`;
      const acceptNotificationLink = `/squad/${squadId}`;

      await sql`
        INSERT INTO notifications (user_id, type, message, from_user_id, link)
        VALUES (
          ${invite[0].from_user_id}::integer, 
          ${acceptNotificationType}::text, 
          ${acceptNotificationMessage}::text,
          ${userId}::integer,
          ${acceptNotificationLink}::text
        )
      `;

      message = `Você entrou no squad "${invite[0].squad_name}"!`;
    } else {
      // Criar notificação para quem convidou
      const declineNotificationType = 'squad_invite_declined';
      const declineNotificationMessage = `Seu convite para o squad "${invite[0].squad_name}" foi recusado.`;
      const declineNotificationLink = `/squad/${squadId}`;

      await sql`
        INSERT INTO notifications (user_id, type, message, from_user_id, link)
        VALUES (
          ${invite[0].from_user_id}::integer, 
          ${declineNotificationType}::text, 
          ${declineNotificationMessage}::text,
          ${userId}::integer,
          ${declineNotificationLink}::text
        )
      `;

      message = `Você recusou o convite para o squad "${invite[0].squad_name}".`;
    }
